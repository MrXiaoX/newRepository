...
//			user.setBaidu(userVO.getBaidu());
	...
	@RequestMapping(value="/checkUserPhone",method=RequestMethod.GET,produces= "application/json")
	public @ResponseBody
	Dto checkUserPhone(
			@ApiParam(name="name",value="被检查的用户的手机号",defaultValue="test@bdqn.cn")
			@RequestParam String name) {
		try {
			if(!validPhone(name))
				return  DtoUtil.returnFail("请使用正确的正确的手机号码",ErrorCode.AUTH_ILLEGAL_USERCODE);
	...
	@ApiOperation(value="手机激活",httpMethod = "POST",
			protocols = "HTTP", produces = "application/json",
			response = Dto.class,notes="手机激活")
	@RequestMapping(value="/validatephone",method=RequestMethod.PUT,produces= "application/json")
	public @ResponseBody
	Dto activatePhone(
			@ApiParam(name="user",value="手机号码",defaultValue="15573520755")
			@RequestParam String user,
			@ApiParam(name="code",value="验证码",defaultValue="018f9a8b2381839ee6f40ab2207c0cfe")
			@RequestParam String code){
		try {
